#!/usr/bin/env python3
"""
frontier_v2.py — Information‑limited (IL) mean‑variance frontier with Stage 2 concentration
...
def _concat_stats(stats: Dict[str, np.ndarray]) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, int, np.ndarray, np.ndarray, np.ndarray, np.ndarray, list, np.ndarray, list]:
    ...
    # Extract Stage 1 weights and Stage 2 concentration levels
    strategy_map = stats.get("strategy_map", [])
    Wm1 = stats.get("Wm1", _weight_splits(UNITS, NUM_PILES))
    M2_LEVELS = stats.get("M2_LEVELS", ['keep', 1, 2, 3])

    return g1, g2, g1sq, g2sq, g12, cnt, Wm1, ace_hits, king_hits, queen_hits, strategy_map, Wm1, M2_LEVELS
...
def _worker_chunk_il(base_seed, round_start, rounds_chunk, signal_type, n_sig, sp, scale_param, rmax_tables, joint_tables, prior_rmax, r2_marginal, debug_excel=False):
    ...
    stats = dict(
        ...
        count=0,
        strategy_map=strategy_map,  # Track (stage1_idx, m2_level) for each strategy
        Wm1=Wm1,
        M2_LEVELS=M2_LEVELS,
    )
    ...
    for i in range(rounds_chunk):
        ...
        stats["sum_g2_sq"] += g2_all * g2_all
        stats["sum_g12"] += g1_expanded * g2_all
        ...
        # Replicate for each Stage 2 variant
        stats["ace_hits"] += np.repeat(ace_hit_mask_s1, len(M2_LEVELS)).astype(int)
        stats["king_hits"] += np.repeat(king_hit_mask_s1, len(M2_LEVELS)).astype(int)
        stats["queen_hits"] += np.repeat(queen_hit_mask_s1, len(M2_LEVELS)).astype(int)
...
def _save_bins_npz(out_path: pathlib.Path, sd_levels_by_n, best_means_by_n, best_weights_by_n, best_m2_levels_by_n, best_ace_hits_by_n, best_king_hits_by_n, best_queen_hits_by_n, meta: dict):
    payload = dict(
        ...
        best_m2_levels_by_n=np.array(best_m2_levels_by_n, dtype=object),
...
def simulate_and_save_frontier(seed_int, rounds, max_signals, procs, params, stage1_alloc, out_dir: pathlib.Path, signal_type: str, debug_excel=False):
    ...
    best_means_by_n = []
    best_weights_by_n = []
    best_m2_levels_by_n = []  # NEW: Track m2 concentration level
    best_ace_hits_by_n = []
    best_king_hits_by_n = []
    ...
    for n_sig in range(int(max_signals) + 1):
        ...
        if stats is None:
            ...
            best_m2_levels_by_n.append([])
            best_ace_hits_by_n.append(np.array([], int))
            best_king_hits_by_n.append(np.array([], int))
            best_queen_hits_by_n.append(np.array([], int))
            continue
        g1, g2, g1sq, g2sq, g12, cnt, Wm1, ace_hits, king_hits, queen_hits, strategy_map, _, M2_LEVELS = _concat_stats(stats)
        ...
        if max_bin >= 0:
            for b in range(max_bin + 1):
                ...
                # Extract strategy metadata: (i1, m2_level)
                i1, m2_level = strategy_map[sel]
                sd_levels.append(float(b) * SD_STEP)
                best_means.append(float(mean_net[sel]))
                best_weights.append(Wm1[i1])  # Store Stage 1 weights
                best_m2_levels.append(m2_level)  # Store Stage 2 concentration level
        ...
        best_m2_levels_by_n.append(best_m2_levels)
