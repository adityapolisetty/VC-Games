...
from web_wrangler_fixed_v3 import run_ui, start_persistent_server, reset_game_state  # UI server only - using v3 threaded version
...
# ===============================
# Multi-threaded game handler
# ===============================
def handle_player_game(http_session_id, team_name, game_type, mode, cost, ace_pay):
    """Handle a single player's game in a dedicated thread"""
    session_id = None  # Database session ID
    try:
        print(f"[game {http_session_id[:8]}] Starting game for '{team_name}' (mode: {mode})")

        # Deal the board (after player has entered)
        game_seed = np.random.randint(0, 1_000_000)
        df = draw_deck(n_cards=9, seed=game_seed)
        print(f"[game {http_session_id[:8]}] Board dealt with seed: {game_seed}")
        print(f"[game {http_session_id[:8]}] 9 piles created:")
        ...
        # ---- Stage 1 ----
        print(f"[game {http_session_id[:8]}] Starting Stage 1 with signal_mode={mode}")
        ...
        act = run_ui(1, df, wallet, signal_mode=mode, signal_cost=cost, session_id=http_session_id)
        if act is None:
            # Player closed browser or restarted - delete abandoned session
            print(f"[game {http_session_id[:8]}] Stage 1 returned None - game abandoned before submission")
            if session_id:
        ...
        # ---- Stage 2 ----
        print(f"[game {http_session_id[:8]}] Stage 1 complete. Starting Stage 2 with wallet=£{wallet:.2f}")
        # Only allow investing in piles that were invested in Stage 1
        act = run_ui(2, df, wallet, signal_mode=mode, signal_cost=cost, stage1_invested=stage1_invested_ids, stage_history=stage_history, session_id=http_session_id)
        print(f"[game {http_session_id[:8]}] Stage 2 UI returned, processing actions")
        if act is None:
        ...
        _ = run_ui(stage=3, df=df, wallet=wallet, results=stats, signal_mode=mode, signal_cost=cost, session_id=http_session_id)
        ...
        print(f"[ERROR {http_session_id[:8]}] Restarting game in 3 seconds...")
        import time
        time.sleep(3)
    finally:
        # Remove this thread from active games
        with games_lock:
            if http_session_id in active_games:
                del active_games[http_session_id]
        print(f"[game {http_session_id[:8]}] Thread complete, cleaned up session")
...
if __name__ == "__main__":
    ...
    # Main loop: just keep server alive, games run in threads
    print("[main] Server running. Players will spawn individual game threads.")
    print("[main] Press Ctrl+C to stop server.")
    try:
        while True:
            import time
            time.sleep(1)
        ...
        print(f"[main] Active games: {len(active_games)}")
