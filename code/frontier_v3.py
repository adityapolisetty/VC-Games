#!/usr/bin/env python3
"""
frontier_v3.py — Full enumeration of Stage 1 AND Stage 2 weight combinations

Key changes from v2:
- ALPHA_GRID: 4 values (0, 0.333, 0.667, 1.0)
- UNITS: 4 (weight discretization)
- Single signal_cost (3), single scale_param (0.25), 2 signal types
- Stage 1: Full enumeration across all m values (1-9 piles) = 36,288 strategies
- Stage 2: Full enumeration of ALL weight combinations within Stage 1 support
  - For Stage 1 strategy with support size m, enumerate all C(4+m-1, m-1) weight vectors
  - Total: ~4.9M strategy combinations per file

Estimated runtime: ~20 minutes (8 files vs 252, but 4× more strategies per file)
Total work: 13% of v2 (39.5M vs 301.6M strategy evaluations)
"""
...
UNITS = 9  # Reduced from 9 for faster enumeration
SD_STEP = 10  # percentage points
...
def _concat_stats(stats: Dict[str, np.ndarray]) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, int, np.ndarray, np.ndarray, np.ndarray, np.ndarray, list, np.ndarray]:
    ...
    # Extract Stage 1 weights and Stage 2 weights
    strategy_map = stats.get("strategy_map", [])
    Wm1 = stats.get("Wm1", np.zeros((0, NUM_PILES), float))
    Wm2_all = stats.get("Wm2_all", np.zeros((0, NUM_PILES), float))

    return g1, g2, g1sq, g2sq, g12, cnt, Wm1, ace_hits, king_hits, queen_hits, strategy_map, Wm2_all
...
def _worker_chunk_il(base_seed, round_start, rounds_chunk, signal_type, n_sig, sp, scale_param, rmax_tables, joint_tables, prior_rmax, r2_marginal, debug_excel=False):
    """
    Worker for IL frontier computation with FULL Stage 2 enumeration.

    Stage 1: Full enumeration over all m values (1-9 piles) with UNITS discretization
    Stage 2: For EACH Stage 1 strategy, enumerate ALL weight combinations within its support

    This creates ~4.9M strategy combinations per file (vs 1.2M in v2), but we run
    only 8 files (vs 252 in v2), resulting in 13% of v2's total work.
    """
    ...
    strategy_map = []  # List of (stage1_idx, stage2_weights) tuples
    Wm2_all_list = []  # Collect all Stage 2 weight vectors

    for i1 in range(Ns1):
        support_mask = (Wm1[i1] > 0)  # Which piles does Stage 1 invest in?
    ...
    Wm2_all = np.array(Wm2_all_list, float)  # Shape: (Ns_total, NUM_PILES)
    ...
    stats = dict(
        ...
        count=0,
        strategy_map=strategy_map,  # Track (stage1_idx, stage2_idx) for each strategy
        Wm1=Wm1,
        Wm2_all=Wm2_all,
    )
    ...
    for i in range(rounds_chunk):
        ...
        # Expand g1 to match strategy_map length
        g1_expanded = np.array([g1_all[i1] for i1, _ in strategy_map], float)
        ...
        # Expand to match strategy_map
        ace_hit_expanded = np.array([ace_hit_mask_s1[i1] for i1, _ in strategy_map], dtype=int)
        king_hit_expanded = np.array([king_hit_mask_s1[i1] for i1, _ in strategy_map], dtype=int)
        queen_hit_expanded = np.array([queen_hit_mask_s1[i1] for i1, _ in strategy_map], dtype=int)

        stats["ace_hits"] += ace_hit_expanded
        stats["king_hits"] += king_hit_expanded
        stats["queen_hits"] += queen_hit_expanded
...
def simulate_and_save_frontier(seed_int, rounds, max_signals, procs, params, stage1_alloc, out_dir: pathlib.Path, signal_type: str, debug_excel=False):
    ...
    for n_sig in range(int(max_signals) + 1):
        ...
        if stats is None:
            ...
            continue
        g1, g2, g1sq, g2sq, g12, cnt, Wm1, ace_hits, king_hits, queen_hits, strategy_map, Wm2_all = _concat_stats(stats)
        ...
        if max_bin >= 0:
            for b in range(max_bin + 1):
                ...
                i1, i2 = strategy_map[sel]
                sd_levels.append(float(b) * SD_STEP)
                best_means.append(float(mean_net[sel]))
                best_weights_s1.append(Wm1[i1])  # Store Stage 1 weights
                best_weights_s2.append(Wm2_all[i2])  # Store Stage 2 weights
...
def run_sweep(base_seed, rounds, max_signals, procs_inner, out_dir,
    ...
    # Reduced parameter grid for v3
    SIGNAL_COSTS = [3]  # Single value
    SCALE_PARAMS = [0.25]  # Single value
    SCALE_PAYS = [0, 1]  # Keep both for comparison
...
def main():
    ...
    Reduced parameter space (4 alphas, single signal_cost/scale_param) but
    comprehensive Stage 2 weight exploration within each Stage 1's support.
    """
    ...
    ap.add_argument("--sweep_out", type=str, default="frontier_output_v3", help="Output directory for sweep")
